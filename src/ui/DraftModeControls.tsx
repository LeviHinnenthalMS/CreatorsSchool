...
import type { GLOBAL_MODULES_QUERY_RESULT } from '@/sanity/types'

export default function DraftModeControls({
	globalModules,
}: {
	globalModules?: GLOBAL_MODULES_QUERY_RESULT
}) {
	...
	const filteredGlobalModules = globalModules
		?.filter(({ path, excludePaths: ex }) => {
			...
			return (
				p === '*' ||
				(curr.startsWith(p) && !ex?.some((e) => !!e && curr.startsWith(e)))
			)
		})
		.sort((a, b) => (a.path ?? '').localeCompare(b.path ?? ''))

	return (
		<details className="frosted-glass bg-warning/90 text-ink-inverse fixed right-4 bottom-0 rounded-t text-xs shadow-xl not-hover:opacity-50 open:opacity-100">
